...
import { buildEsConditionSentence, EValueHowToSetUp, getConditionsByGroup } from "@isettingkit/business-rules";
...
const withConditionSentences = (
  ...
  const groups = (getConditionsByGroup(decisionClone) || {}) as Record<string, unknown>;
    ...
    orderedKeys.map((groupKey) => {
      ...
      const mapped = list.map((condition: any, idx: number) => {
        ...
        const how = normalizeHowToSet(
          condition.howToSetTheCondition ?? condition.valueUse ?? EValueHowToSetUp.EQUAL,
        );
