...
function useStepNavigation({ groups }: IUseStepNavigation) {
  ...
  const nextIndex = currentIndex >= 0 && currentIndex < sequence.length - 1 ? currentIndex + 1 : -1;
  ...
  const handleBack = React.useCallback(() => {
    ...
    const isFirstLinkOfGroup =
      here.kind === "link" &&
      (prev.groupId !== here.groupId || (prev.kind === "group" && prev.groupId === here.groupId));

    if (isFirstLinkOfGroup && prev.kind === "group" && prev.groupId === here.groupId) {
      navigate(prev.path);
      return;
    }
    navigate(prev.path);
